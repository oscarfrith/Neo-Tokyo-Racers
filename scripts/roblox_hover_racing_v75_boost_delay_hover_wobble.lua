--[[
	Hover Racing V2 - V75 Boost Delay + Hover Wobble
	Paste this whole file into the Roblox Studio Command Bar while NOT play-testing.

	This keeps the working V74 camera/driving setup, then adds:
	- a 0.5 second boost recharge delay
	- editable boost attributes on Boost module templates
	- a subtle low-speed hover wobble that fades out by 20 MPH
	...
	- Adds ReplicatedStorage.HOVER_RACING_V2_KIT.CONFIG.DRIVING_MECHANICS_EditAttributes.
	- Adds ReplicatedStorage.HOVER_RACING_V2_KIT.CONFIG.HOVER_WOBBLE_EditAttributes.
...
local function requireChild(parent, name)
	...
	if not child then
		error(("[V75] Missing %s under %s"):format(name, parent and parent:GetFullName() or "nil"))
	end
	return child
end
...
local mechanicsConfig = ensureFolder(configRoot, "DRIVING_MECHANICS_EditAttributes")
local wobbleConfig = ensureFolder(configRoot, "HOVER_WOBBLE_EditAttributes")
...
local wobbleDefaults = {
	WobbleEnabled = true,
	WobbleAmountDegrees = 1.15,
	WobbleSpeed = 1.15,
	WobbleRandomiseAmount = 0.65,
	WobbleFadeOutMph = 20,
	WobblePitchMultiplier = 0.75,
	WobbleRollMultiplier = 1.0,
	WobbleSmoothing = 4.5,
}

for name, value in pairs(cameraDefaults) do
	if cameraConfig:GetAttribute(name) == nil then
		cameraConfig:SetAttribute(name, value)
	end
end

for name, value in pairs(mechanicsDefaults) do
	if mechanicsConfig:GetAttribute(name) == nil then
		mechanicsConfig:SetAttribute(name, value)
	end
end

for name, value in pairs(wobbleDefaults) do
	if wobbleConfig:GetAttribute(name) == nil then
		wobbleConfig:SetAttribute(name, value)
	end
end

local function isBoostModule(model)
	...
	return moduleType == "Boost"
		or string.find(name, "module_boost", 1, true) ~= nil
		or string.find(name, "boost", 1, true) ~= nil and string.find(parentName, "boost", 1, true) ~= nil
end

local boostTemplatesUpdated = 0
for _, descendant in ipairs(kit:GetDescendants()) do
	if isBoostModule(descendant) then
		if descendant:GetAttribute("Boost") == nil then descendant:SetAttribute("Boost", 32) end
		if descendant:GetAttribute("BoostDuration") == nil then descendant:SetAttribute("BoostDuration", 2) end
		if descendant:GetAttribute("BoostRecharge") == nil then descendant:SetAttribute("BoostRecharge", 9) end
		if descendant:GetAttribute("BoostRechargeDelay") == nil then descendant:SetAttribute("BoostRechargeDelay", 0.5) end
		if descendant:GetAttribute("BoostNotes") == nil then
			descendant:SetAttribute("BoostNotes", "Boost controls thrust power, BoostDuration controls burn time, BoostRecharge controls recharge speed, BoostRechargeDelay controls delay before recharge starts.")
		end
		boostTemplatesUpdated += 1
	end
end
...
if drivingModule then
	drivingModule:SetAttribute("HoverRacingVersion", "V75_BoostDelayHoverWobble")
end
...
source = stripMarkedBlock(source, "-- V75_BOOST_WOBBLE_DRIVING_BEGIN", "-- V75_BOOST_WOBBLE_DRIVING_END")

if not string.find(source, "local function startDriving()", 1, true) then
	error("[V75] Could not find local function startDriving(). No client changes were applied.")
end
...
log("Installed V47-style driving module, boost delay, hover wobble, and tiny client wrapper. UI/customisation/server systems were not touched.")
log("Boost module templates touched: " .. tostring(boostTemplatesUpdated))
print("Hover Racing V75 boost delay + hover wobble complete. Run in Edit mode, then Play fresh and spawn the vehicle.")
